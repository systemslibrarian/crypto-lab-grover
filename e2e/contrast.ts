 ...
 *    files the node under `incomplete`. The shared header paints two
 *    `linear-gradient` grid rules over its own solid fill, so every label and
 *    control in the top bar composites onto something axe will not measure.
 *  - text faded by an ancestor's `opacity` — axe reads the declared `color`,
 *    which is not the colour that lands on screen. `.cl-hero-sub` is exactly
 *    that shape here: `opacity: .85` applied on top of `color: var(--text-dim)`,
 *    so the ratio axe reports is one the page never paints. Note this lab
 *    collapses its animations to 0.01ms under reduced motion, which preserves
 *    end states.
 *
 ...
 *
 *  - TEXT SCROLLED OUT OF A CLIPPING ANCESTOR PAINTS NOTHING. `.hash-table` and
 *    `.speedup-table` set `white-space: nowrap` inside `overflow-x: auto`
 *    wrappers, so at 380px most of their cells are scrolled out of the wrapper's
 *    client box. Content scrolled past that box is not dimmed or partly drawn —
 *    it is absent from the frame, and asking what colour it sits on has no
 *    answer. Its rect is still to the right of every ancestor's box, so the
 ...
 *    same cells are visible and measured for real.
 *
 ...
 *    draws its two figures (the rotation diagram and the probability curve) on
 *    `<canvas>`, and its only SVG is the header's `aria-hidden` hamburger icon —
 *    so this rule finds nothing here today. It is kept because it is cheap and
 *    silent when unused: an `<svg>` draws a shape as a `<rect>` and then its
 *    label as a following `<text>`, so a label's backdrop is a PRECEDING SIBLING
 ...
 *    the text box is composited first.
 */
...
export async function auditContrast(page: Page): Promise<ContrastFailure[]> {
  return page.evaluate(() => {
     ...
     *
     * A hand-rolled `rgba()` regex is not enough here: the shared header mixes
     * its accent into surfaces with `color-mix()` (the hero's "why it matters"
     * wash and the top bar's chip borders), and Chromium reports those
     ...
     *
     * This page renders one amplitude bar plus one label per basis state (up to
     * 2^20 is selectable, and n = 8 alone is 256 of each), a 100-cell
     * measurement grid, and the glossary's 28 terms — every one of them walks
    ...
    const isVisible = (el: Element): boolean => {
      ...
      // paints nothing. This page has three disclosures (Grover vs Shor, the
      // sources table, the glossary), each of whose open state is a real state
      // the gate scans explicitly.
      ...
      // `top: -3rem` and the lab's own `.skip-link` at `top: -100%`, each
     ...
     * it, the header's `aria-hidden` theme glyph (☀/☾) and the buttons' own
     * decorative marks are measured as if each took its element's declared
     * `color` — but a glyph rendered from an emoji font paints in its own
     * multicolour, so that ratio describes nothing on screen.
     */
     ...
     *
     * This lab has no SVG figures, so the rule is inert here — but it is the
     * kind of mistake that is cheap to guard: interpolating a string ARRAY into
     * a template literal (`<g>${dots}</g>`) makes JS join it with commas,
     * leaving a run of stray "," text nodes inside the group. They are
     * invisible on screen, but a walk that measures them reports a 1:1 failure
     * against the panel — a ratio describing ink that was never laid down.
     */
